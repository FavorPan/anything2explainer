...
/**
 * 字幕：Noto Sans SC 700 44px、居中 x=640、CSS top 637（墨迹 y644–684）；样式按主题（theme.ts）：
 * dark = 白字 + 4px 黑描边（16+8+4 方向 text-shadow 环，避免 -webkit-text-stroke 的尖角刺）、靠雾底衬托；
 * mint = 墨色字无描边（stroke 0 → 不输出 textShadow）。无底框、进出单帧硬切。
 * 条目由 scripts/tts_build.py 从配音词边界生成（每块中文 ≤16 字 / 英文 ≤48 字符）。
 */
export const SUB_STYLE = {fontSize: 44, weight: 700, top: 637, color: THEME.subColor, stroke: THEME.subStroke, strokeColor: THEME.subStrokeColor};
...
export const strokeShadow = (w = SUB_STYLE.stroke, col = SUB_STYLE.strokeColor) => (w > 0 ? [...ring(w, 16, col), ...ring(w * 0.6, 8, col), ...ring(w * 0.3, 4, col)].join(', ') : 'none');
