...
| **主角登场（高光时刻·登场型）** | 清场 → 三轮 `LightSweep` → `StageLine` → `GhostText` 预示 → 白闪 + 宽体大字 `GlitchIn`（rgbSplit）→ 脉冲 → 副标 slideUp → 拆词 Pill SoftIn | Audiowide 150 + 主题硬投影；全流程 ≥90 帧（`fx.tsx` `SET_PIECE`） | SC08、SC35 · `G2/SC08.tsx`、`G7/SC35.tsx` |
