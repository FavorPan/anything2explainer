...
- **片头要有口播**：第一行（第一个 `# CHAPTER` 之前）写读标题的一句——直接读片名或一句钩子，≤20 字、一句即可；tts_build 把它识别成片头口播 T 句（id `T01`，不占 S 编号，分镜令牌不用重排），压在片头卡上（LEAD 后起播），字幕带照常出，片头自动延长到它讲完。无声干等 2 秒的片头按缺陷处理。
