...
| 常驻层 | 44px 白字黑边字幕、底部章节进度条、顶部胶囊 HUD、可选流程轨、可选片尾署名行（上游默认印 `built by Anything2Explainer skill`；本库默认印 `built by FavorPan`） |
...
章数按内容定、受时长约束：<3 分钟单章（不出章节卡），3–5 分钟 3–5 章且每章 ≥45 s，5–8 分钟 4–6 章；进度条按解说词声明的章数等宽分段。解说词用空行分段，一段就是一个镜头：段内句间只隔 10 帧、段末 30 帧，停顿落在画面切换处，每个镜头末拍落位后能停 1–1.5 s 再切；成片比纯语音长 5–8% 是设计内的。
...
**能换视觉风格吗？**
只做这一种风格，是有意为之，唯一的开关是幕底：`src/config.ts` 的 `bg`（默认 `'dots'` 点阵波，可选 `'stars'` 星点雾底）。其他要换就改 `reference/style-guide.md` 和 `src/ui.tsx`，镜头代码只用这些图元。
